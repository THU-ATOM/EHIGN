...
class DTIPredictor(nn.Module):
    def __init__(
        ...
        for _ in range(layer_num):
            convl = EGNNConv(hidden_feat_size, hidden_feat_size)
            convp = EGNNConv(hidden_feat_size, hidden_feat_size)
            convlp = EGNNConv(hidden_feat_size, hidden_feat_size)
            convpl = EGNNConv(hidden_feat_size, hidden_feat_size)
        ...
        self.inter_atompairs = AtomAtomAffinities(
            hidden_feat_size, edge_feat_size, hidden_feat_size
        )
    ...
    def forward(self, bg):
        ...
        bond_feats = {
            ("ligand", "intra_l", "ligand"): bond_feats[
                ("ligand", "intra_l", "ligand")
            ][:, -1].unsqueeze(-1)
            * 10,
            ("pocket", "intra_p", "pocket"): bond_feats[
                ("pocket", "intra_p", "pocket")
            ][:, -1].unsqueeze(-1)
            * 10,
            ("ligand", "inter_l2p", "pocket"): bond_feats[
                ("ligand", "inter_l2p", "pocket")
            ][:, -1].unsqueeze(-1)
            * 10,
            ("pocket", "inter_p2l", "ligand"): bond_feats[
