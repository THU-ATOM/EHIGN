...
class IGAT(nn.Module):
    def __init__(self, node_dim, hidden_dim, d_count=9):
        ...
        self.gconv1 = GATConv(
            hidden_dim, hidden_dim, heads=3, concat=False, edge_dim=hidden_dim
        )
        self.gconv2 = GATConv(
            hidden_dim, hidden_dim, heads=3, concat=False, edge_dim=hidden_dim
        )
...
def _rbf(D, D_min=0.0, D_max=20.0, D_count=16, device="cpu"):
    ...
    Returns an RBF embedding of `torch.Tensor` `D` along a new axis=-1. That is,
    if `D` has shape [...dims], then the returned tensor will have shape
    [...dims, D_count].
    """
