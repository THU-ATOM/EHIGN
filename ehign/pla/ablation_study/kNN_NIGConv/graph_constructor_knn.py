...
def mols2graphs(complex_path, label, save_path, dis_threshold=5.0, knn=3):
    try:
        ...
        edge_index_l2p, edge_attr_l2p = knn_graph(
            atom_num_l, edge_index_l2p, edge_attr_l2p, knn
        )
        edge_index_p2l, edge_attr_p2l = knn_graph(
            atom_num_p, edge_index_p2l, edge_attr_p2l, knn
        )
    ...
    def _pre_process(self):
        ...
        for i, row in data_df.iterrows():
            ...
            graph_path = os.path.join(
                complex_dir, f"{graph_type}_{self.knn}NN-{cid}.dgl"
            )
            complex_path = os.path.join(
                complex_dir, f"{cid}_{int(self.dis_threshold)}A.rdkit"
            )
