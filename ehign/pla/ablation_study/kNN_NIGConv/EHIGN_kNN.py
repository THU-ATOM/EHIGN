...
class DTIPredictor(nn.Module):
    def __init__(
        self, node_feat_size, edge_feat_size, hidden_feat_size, layer_num=3
    ):
        ...
        for _ in range(layer_num):
            ...
            # use kNN graphs, so we can replace mean aggregation with sum aggregation
            # that is, replace NIGConv with CIGConv
            convlp = CIGConv(hidden_feat_size, hidden_feat_size)
            convpl = CIGConv(hidden_feat_size, hidden_feat_size)
        ...
        self.inter_atompairs = AtomAtomAffinities(
            hidden_feat_size, hidden_feat_size, hidden_feat_size
        )
    ...
    def forward(self, bg):
        ...
        bond_feats = {
            ("ligand", "intra_l", "ligand"): self.lin_edge_ll(
                bond_feats[("ligand", "intra_l", "ligand")]
            ),
            ("pocket", "intra_p", "pocket"): self.lin_edge_pp(
                bond_feats[("pocket", "intra_p", "pocket")]
            ),
            ("ligand", "inter_l2p", "pocket"): self.lin_edge_lp(
                bond_feats[("ligand", "inter_l2p", "pocket")]
            ),
            ("pocket", "inter_p2l", "ligand"): self.lin_edge_pl(
                bond_feats[("pocket", "inter_p2l", "ligand")]
            ),
        }
        ...
        return (atompairs_lp - bias_lp).view(-1), (atompairs_pl - bias_pl).view(
            -1
        )
...
class BiasCorrectionLigandPocket(nn.Module):
    def __init__(self, node_feat_size, edge_feat_size, hidden_feat_size):
        ...
        self.lin_att = nn.Sequential(nn.PReLU(), nn.Linear(hidden_feat_size, 1))
    ...
    def forward(self, g):
        with g.local_scope():
            ...
            scores = edge_softmax(
                g["inter_l2p"], g.edges["inter_l2p"].data["w"]
            )
...
class BiasCorrectionPocketLigand(nn.Module):
    ...
    def forward(self, g):
        with g.local_scope():
            ...
            scores = edge_softmax(
                g["inter_p2l"], g.edges["inter_p2l"].data["w"]
            )
...
class FC(nn.Module):
    def __init__(self, d_graph_layer, d_FC_layer, n_FC_layer, dropout, n_tasks):
        ...
        for j in range(self.n_FC_layer):
            if j == 0:
                self.predict.append(
                    nn.Linear(self.d_graph_layer, self.d_FC_layer)
                )
