...
from dgllife.model.gnn.schnet import Interaction, RBFExpansion
from HGC import HeteroGraphConv


class DTIPredictor(nn.Module):
    def __init__(
        self, node_feat_size, hidden_feat_size, layer_num=3, cutoff=6.0, gap=0.1
    ):
        ...
        for _ in range(layer_num):
            convl = Interaction(
                hidden_feat_size, len(self.rbf.centers), hidden_feat_size
            )
            convp = Interaction(
                hidden_feat_size, len(self.rbf.centers), hidden_feat_size
            )
    ...
    def forward(self, bg):
        ...
        bond_feats = {
            ("ligand", "intra_l", "ligand"): self.rbf(
                bond_feats[("ligand", "intra_l", "ligand")][:, -1].unsqueeze(-1)
                * 10
            ),
            ("pocket", "intra_p", "pocket"): self.rbf(
                bond_feats[("pocket", "intra_p", "pocket")][:, -1].unsqueeze(-1)
                * 10
            ),
            ("ligand", "inter_l2p", "pocket"): self.rbf(
                bond_feats[("ligand", "inter_l2p", "pocket")][:, -1].unsqueeze(
                    -1
                )
                * 10
            ),
            ("pocket", "inter_p2l", "ligand"): self.rbf(
                bond_feats[("pocket", "inter_p2l", "ligand")][:, -1].unsqueeze(
                    -1
                )
                * 10
            ),
